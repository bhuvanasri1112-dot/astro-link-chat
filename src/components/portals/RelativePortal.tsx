...
import { Heart, MessageSquare, Users, LogOut, Mail } from "lucide-react";
...
const RelativePortal = ({ profile }: RelativePortalProps) => {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="cosmic-card m-4 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="cosmic-glow p-2 rounded-full bg-secondary/10">
            <Heart className="w-6 h-6 text-secondary" />
          </div>
          <div>
            <h2 className="font-bold text-lg">{profile.full_name}</h2>
            <p className="text-sm text-muted-foreground">{profile.relationship || "Family Member"}</p>
          </div>
        </div>
      ...
      <div className="flex gap-2 px-4">
        <Button
          ...
          className={activeTab === "chat" ? "bg-gradient-to-r from-secondary to-accent" : ""}
        ...
        <Button
          ...
          className={activeTab === "messages" ? "bg-gradient-to-r from-secondary to-accent" : ""}
        ...
        <Button
          ...
          className={activeTab === "connections" ? "bg-gradient-to-r from-secondary to-accent" : ""}
